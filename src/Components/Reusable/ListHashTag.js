  ...
  return (
    <div>
      <ErrorModal
        ...
        show={showError}
      />
      {posts && popular && popular[0] && posts[0] && (
        <div className="listHash-wrapper">
          <div className="list-hash-wrapper1">
            <div className="list-hash-image-wrapper">
              <img alt="" src={popular[0].image} />
            </div>
            <NavLink
              to={`hashtag/${shortHash}`}
              className="list-hash-names-wrapper"
            >
              <p>{hashTag}</p>
              <p style={{ color: '#8e8e8e', fontWeight: '400' }}>
                {posts.length} posts
              </p>
            </NavLink>
          </div>
          <div className="list-person-wrapper2">
            {followedArr.includes(hashTag) ? (
              <button
                onClick={() => hashHandler(hashTag)}
                className="list-following-button"
              >
                Following
              </button>
            ) : (
