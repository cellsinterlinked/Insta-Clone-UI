...
const ListHashTag = ({hashTag, hashHandler, followedArr}) => {
  ...
  useEffect(() => {
    async function getPosts() {
      const res = await api.get(`posts/hash/${hashTag}`)
    ...
    <>
     {posts && popular && <div className="listHash-wrapper">
      <div className="list-hash-wrapper1">
        ...
        <NavLink to={`hashtag/${hashTag}`} className="list-hash-names-wrapper">
          <p >{hashTag}</p>
